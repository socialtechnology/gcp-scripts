...
def get_cloud_run_info(project=None):
    ...
    for service in services:
        ...
        # New columns
        cpu_throttling_annotation = annotations.get("run.googleapis.com/cpu-throttling", "")
        cpu_allocation = "CPU allocation only during request processing" if cpu_throttling_annotation.lower() != "false" else "CPU is always allocated"

        execution_environment_annotation = annotations.get("run.googleapis.com/execution-environment", "gen1")
        execution_environment = "First Generation" if execution_environment_annotation.lower() in ["gen1", ""] else "Second Generation"
